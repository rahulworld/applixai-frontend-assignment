import { axisClasses, barElementClasses, LineChart, useDrawingArea } from "@mui/x-charts";
...
const TrendLineChart = ({ data, xAxisDataKey, yAxisDataKey }: { data: DatasetType; xAxisDataKey: string; yAxisDataKey: string; }) => {
  return (<LineChart
    className="w-full"
    dataset={data}
    xAxis={[
      {
        ...
        valueFormatter: (month, context) => {
          ...
          return context.location === 'tick'
            ? `${dateObj.toLocaleString('en-us', { month: 'short' }).slice(0, 3)}, ${dateObj.getDate()} \n${dateObj.getFullYear()}`
            : `${dateObj.toLocaleString('en-us', { month: 'short' })}, ${dateObj.getDate()} ${dateObj.getFullYear()}`;
        }
      },
    ]}
    series={[
